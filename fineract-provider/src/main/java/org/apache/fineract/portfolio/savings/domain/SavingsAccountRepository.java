...
public interface SavingsAccountRepository extends JpaRepository<SavingsAccount, Long>, JpaSpecificationExecutor<SavingsAccount> {
    ...
    @Query("select s_acc.accountNumber from SavingsAccount s_acc where s_acc.status = :status and s_acc.depositType = 100 and s_acc.product.isMainProduct = true")
    List<String> findMainSavingAccountNumberByStatus(@Param("status") Integer status);
    
    @Query("select s_acc.accountNumber from SavingsAccount s_acc where s_acc.status = :status and s_acc.depositType = 100 and s_acc.product.isMainProduct = false")
    List<String> findSubSavingAccountNumberByStatus(@Param("status") Integer status);
    ...
    @Query("select sa from SavingsAccount sa where sa.accountNumber = :accountNo and sa.depositType = :depositAccountTypeId")
    SavingsAccount findByAccountNoAndDepositAccountType(@Param("accountNo") String accountNo,
            @Param("depositAccountTypeId") Integer depositAccountTypeId);
    ...
    SavingsAccount findAccountByAccountNumber(@Param("accountNumber") String accountNumber);
